...
internal typealias BoundsCheckAnalysisResult = Pair<Boolean, IrValueSymbol?>
...
class KonanBCEForLoopBodyTransformer : ForLoopBodyTransformer() {
    private var boundsAreSafe = false
    private var arrayInLoop: IrValueSymbol? = null
    ...
    override fun initialize(context: CommonBackendContext, loopVariable: IrVariable,
        ...
        val analysisResult = analyzeLoopHeader(loopHeader)
        boundsAreSafe = analysisResult.first
        arrayInLoop = analysisResult.second
    }
    ...
    private fun lessThenGetSize(functionCall: IrCall): BoundsCheckAnalysisResult {
        var result = false
    ...
    private fun analyzeLoopHeader(loopHeader: ForLoopHeader): BoundsCheckAnalysisResult {
        arrayInLoop = null
        boundsAreSafe = false
        var result = BoundsCheckAnalysisResult(boundsAreSafe, arrayInLoop)
        when(loopHeader) {
                        ...
                        if (loopHeader.headerInfo.last is IrCall) {
                            ...
                            if (loopHeader.headerInfo.progressionType.getProgressionLastElementFunction == functionCall.symbol) {
                                ...
                                if (nestedLastVariable is IrGetValue && nestedLastVariable.symbol.owner is IrVariable) {
                                    ...
                                    result = checkLastElement(nestedLast!!, loopHeader)
                                }
                            } else {
                                // Simple progression.
                                result = checkLastElement(functionCall, loopHeader)
                            }
                        }
                    }
            ...
            is WithIndexLoopHeader ->
                when(loopHeader.nestedLoopHeader) {
                    is IndexedGetLoopHeader -> {
                        result = BoundsCheckAnalysisResult(true,
                                ((loopHeader.loopInitStatements[0] as? IrVariable)?.initializer as? IrGetValue)?.symbol)
                    }
                    is ProgressionLoopHeader -> result = analyzeLoopHeader(loopHeader.nestedLoopHeader)
                }
        }
        return result
    }
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        if (!boundsAreSafe || arrayInLoop == null)
            return expression
        ...
        if (expression.dispatchReceiver?.type?.isBasicArray() != true ||
                (expression.dispatchReceiver as? IrGetValue)?.symbol != arrayInLoop)
            return expression
