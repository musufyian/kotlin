...
object FirTypeAliasChecker : FirMemberDeclarationChecker() {
    override fun check(declaration: FirMemberDeclaration, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        fun containsTypeParameter(type: ConeKotlinType): Boolean {
            ...
            if (type is ConeClassLikeType && type.lookupTag.toSymbol(context.session) is FirTypeAliasSymbol) {
                for (typeArgument in type.typeArguments) {
                    if (typeArgument is ConeKotlinType && containsTypeParameter(typeArgument)) {
                        return true
                    }
                }
            }
        ...
        if (containsTypeParameter(fullyExpandedType) || (fullyExpandedType is ConeClassErrorType && fullyExpandedType.diagnostic is ConeUnsupportedDynamicType)) {
            reporter.reportOn(
